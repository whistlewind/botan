...
BOTAN_REGISTER_TEST("ec_h2c_xmd", ECC_H2C_XMD_Tests);
...
class ECC_H2C_Tests final : public Text_Based_Test
   ...
   public:
      ...
      Test::Result run_one_test(const std::string& method, const VarMap& vars) override
         ...
         const bool random_oracle = method.find("_RO") != std::string::npos;

         Botan::EC_Group group(group_id);

         const auto point = group.hash_to_curve(hash, random_oracle,
